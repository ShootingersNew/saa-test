...
import { MAX_LABEL_LEN, MAX_LOGIN_LEN, MAX_PASSWORD_LEN } from '@/shared/lib/validation/accountValidation';
...
export function normalizePassword(type: AccountType, pwd: string | null | undefined): string | null {
...
export const fieldStrategies: Record<FieldKey, FieldStrategy> = {
  ...
  type(form) {
    ...
    return {
      ...
      cb: (f) => { if (f.type === 'ldap') f.password = null; },
    };
  },
  login(form) {
  ...
  password(form) {
    ...
    if (password.length > MAX_PASSWORD_LEN) return { error: `Превышена длина > ${MAX_PASSWORD_LEN}` };
